...
export class FirebaseService {
    ...
    return this.http.get(`https://ui-project-d452e-default-rtdb.firebaseio.com/customer/${nationalID}.json`).pipe(
      switchMap((response: any) => {
        if (response && response.password === password) {
          ...
          this.gb.custTelType = this.http.get(`https://ui-project-d452e-default-rtdb.firebaseio.com/customer/${nationalID}/telBillType/.json`);          return of(nationalID);
        } else {
          return this.http.get(`https://ui-project-d452e-default-rtdb.firebaseio.com/serviceProviders/${nationalID}.json`).pipe(
            catchError(() => {
              return this.http.get(`https://ui-project-d452e-default-rtdb.firebaseio.com/admin/${nationalID}.json`);
            }),
  ...
  getServices(providerId: string): Observable<any[]> {
    ...
    return this.http.get<any[]>(url).pipe(
      map((data) => Object.values(data).slice(1)),
    );;
  }



  private serviceProvidersUrl = 'https://ui-project-d452e-default-rtdb.firebaseio.com/serviceProviders/.json';
